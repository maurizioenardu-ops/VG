...
function articleCategoryOptions(db){
  const defaults=['Borse','Portafogli','Cinture','Scarpe','Accessori'];
  const saved=Array.isArray(db?.categorie)?db.categorie:[];
  const found=(db?.articoli||[]).map(a=>String(a?.categoria||'').trim()).filter(Boolean);
  return [...new Set([...defaults, ...saved, ...found])].sort((a,b)=>a.localeCompare(b,'it',{sensitivity:'base'}));
}
function renderCategorySettings(){
  ...
  const savedSet=new Set(Array.isArray(db?.categorie)?db.categorie:[]);
  box.innerHTML=cats.length?cats.map(c=>`<div class="row" style="grid-template-columns:1fr auto"><div class="main"><div class="t">${esc(c)}</div><div class="small">${savedSet.has(c)?'Categoria salvata nelle impostazioni':'Categoria rilevata da articoli già esistenti'}</div></div><div><button class="btn danger smallish" type="button" data-action="deleteCategory" data-name="${esc(c)}">Elimina</button></div></div>`).join(''):'<div class="small">Nessuna categoria salvata.</div>';
  if(hint) hint.textContent='Le categorie salvate compaiono nel menu degli articoli. Se ne elimini una, gli articoli esistenti restano intatti.';
}
...
function deleteCategory(nome){
  ...
  if(!clean) return;
  askConfirm(`Tolgo la categoria "${clean}" dall'elenco rapido?`,()=>{
    const db=loadDB();
    db.categorie=(db.categorie||[]).filter(c=>String(c||'').trim()!==clean);
    if(!saveDB(db)) return;
    renderCategorySettings();
    refreshArticleCategorySelects();
...
function renderArt(){
  ...
  const cat=String(document.getElementById('qArtCat')?.value||'').trim();
  const items=db.articoli.filter(a=>{
    ...
    const okCat=!cat || String(a.categoria||'').trim()===cat;
    return okText && okCat;
...
async function openArtEdit(id){
  ...
  if(!a){
    ['a_cod','a_brand','a_mod','a_desc','a_forn','a_taglia','a_variante','a_colore','a_mis','a_usd','a_promo_price','a_promo_date','a_note','a_post','a_qual','a_eur','a_sell','a_final','a_margin','a_margin_pct']
...
function duplicateCurrentArticle(){
  ...
  document.getElementById('a_brand').value=clone.brand||'';
  ...
  document.getElementById('a_forn').value=clone.fornitore||'';
  ...
  currentArtExistingPics=(clone.foto||[]).slice(0,6);
  renderArtPhotoPrev(currentArtExistingPics);
  renderArtAutoFields();
...
async function saveArt(){
  try{
    ...
    if(selectedFiles.length){
      ...
      const maxNew=Math.max(0, 6 - obj.foto.length - obj.photoIds.length);
      ...
      if(filesToSave.length){
        const uploaded=[];
        const localPhotoIds=[];
        for(const [idx,file] of filesToSave.entries()){
          try{
            if(cloudEnabled()){
              const nextSlot=obj.foto.length + uploaded.length + 1;
              const cloudPath=await uploadArticlePhotoToSupabase(file, code, nextSlot);
            ...
            console.warn('Salvataggio foto cloud fallito, provo in locale', photoErr);
            try{
              const dataUrl=await compressImageFile(file);
              const pid='ph_'+uid();
              await idbSet(pid, dataUrl);
              localPhotoIds.push(pid);
              photoUploadWarning='Una o più foto salvate solo in locale';
            }catch(localErr){
        ...
        obj.foto=[...new Set([...(obj.foto||[]), ...uploaded])].slice(0,6);
...
async function deleteArt(){
  ...
  await deleteArticlePhotosFromSupabase(old?.foto||[]);
