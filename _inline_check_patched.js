...
function defDB(){
  return {version:'v6.3',articoli:[],clienti:[],ordini:[],categorie:[],brands:[],createdAt:new Date().toISOString()};
}
function normalizeDBShape(o){
  ...
  db.ordini=db.ordini.map(x=>({
    ...
    righe:Array.isArray(x?.righe)?x.righe.map(r=>({
      ...r,
...
function mergeOrderRecords(prev,item){
  if(!prev) return {
    ...
    righe:Array.isArray(item?.righe)?item.righe:[],
  ...
  merged.totale=Number(merged?.totale||0);
...
async function syncArticlePhotosToCloud(artId, codice, pics){
  ...
  for(let i=0;i<normalized.length;i++){
    ...
    const isDirectUrl=/^(data:|https?:\/\/|blob:)/i.test(src);
    if(!isDirectUrl){
  ...
  const finalPaths=[...new Set((savedPaths.length ? savedPaths : existingPaths).filter(Boolean))].slice(0,6);
  let deleted=false;
  try{
...
async function pullCloudToLocal(opts={}){
  ...
  try{
    ...
    for (const p of (prod||[])) {
      const validRows = Array.isArray(fotoByProd.get(p.id)) ? fotoByProd.get(p.id).filter(Boolean) : [];
      if (validRows.length) {
        fotoByProd.set(p.id, validRows.slice(0,6));
        continue;
      }
    ...
    const db={version:'v6',createdAt:new Date().toISOString(),
      articoli:(prod||[]).map(p=>{
        ...
        const packed=unpackCloudArticleDescription(p.descrizione||''); const legacy=parseLegacyCloudDescription(packed.plain); const meta=(packed.meta&&typeof packed.meta==='object')?packed.meta:{}; const cleanBrand=p.marca||meta.brand||local?.brand||''; const pulledModel=stripBrandFromText(meta.modello||p.nome||'', cleanBrand) || stripBrandFromText(p.nome||'', cleanBrand) || local?.modello || ''; const pulledDescr=(meta.descrizione||legacy.descrizione||local?.descrizione||'').trim(); const pulledNote=(meta.note||legacy.note||local?.note||'').trim(); const pulledPost=(meta.post||local?.post||'').trim(); const rawSell=Number(p.prezzo_vendita||0); const metaSell=Number(meta.prezzoVendita||0); return {id:p.id,codice:p.sku||'',brand:cleanBrand,modello:pulledModel,categoria:meta.categoria||catMap.get(p.categoria_id)||local?.categoria||'',descrizione:pulledDescr,fornitore:meta.fornitore||local?.fornitore||'',fornitoreLink:meta.fornitoreLink||local?.fornitoreLink||'',taglia:meta.taglia||p.taglia||local?.taglia||'',variante:meta.variante||p.materiale||local?.variante||'',colore:meta.colore||p.colore||local?.colore||'',misura:meta.misura||local?.misura||'',costoUsd:Number(meta.costoUsd||local?.costoUsd||0),costoEur:Number(p.prezzo_acquisto||meta.costoEur||local?.costoEur||0),prezzoVendita:Number(rawSell||metaSell||local?.prezzoVendita||0),promoAttiva:(typeof meta.promoAttiva==='boolean') ? meta.promoAttiva : !!local?.promoAttiva,prezzoPromo:Number(meta.prezzoPromo||local?.prezzoPromo||0),scadenzaPromo:meta.scadenzaPromo||local?.scadenzaPromo||'',post:pulledPost,note:pulledNote,foto:(fotoByProd.get(p.id)||local?.foto||[]).slice(0,6),photoIds:Array.isArray(local?.photoIds)?local.photoIds.filter(Boolean).slice(0,6):[],_ts:Date.now(),_cloud:true};
      }),
      ...
      ordini:(ord||[]).map(o=>{ const noteRaw=o.note||''; const cleanNote=noteRaw.replace(/\s*\[MIS:[^\]]+\]\s*/gi,' ').trim(); const misMatch=noteRaw.match(/\[MIS:([^\]]+)\]/i); const shadow={id:o.id,numeroOrdine:o.numero_ordine,data:o.data_ordine||todayStr(),tracking:o.tracking_code||'',totale:Number(o.totale||0),note:cleanNote,righe:righeByOrd.get(o.id)||[]}; const local=localOrdById.get(o.id)||localOrdByNumero.get(normalizeTextKey(o.numero_ordine||''))||localOrdByFingerprint.get(normalizeOrderFingerprint(shadow)); return {id:o.id,numeroOrdine:ensureOrderNumber({id:o.id,numeroOrdine:o.numero_ordine}),clienteId:o.cliente_id,stato:o.stato==='in_lavorazione'?'Richiesto':(o.stato==='spedito'?'Spedito':(o.stato==='consegnato'?'Consegnato':'Annullato')),data:o.data_ordine||todayStr(),note:cleanNote,mis:misMatch?misMatch[1].trim():'',tracking:o.tracking_code||'',righe:righeByOrd.get(o.id)||[],fotoArticoli:[],fotoManuali:Array.isArray(local?.fotoManuali)?local.fotoManuali.filter(Boolean).slice(0,12):[],orderPhotoIds:Array.isArray(local?.orderPhotoIds)?local.orderPhotoIds.filter(Boolean).slice(0,12):[],foto:[],totale:Number(o.totale||0),_ts:Date.now(),_cloud:true}; })
    };
