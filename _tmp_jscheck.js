...
function defDB(){
  return {version:'v6.2',articoli:[],clienti:[],ordini:[],categorie:[],createdAt:new Date().toISOString()};
}
function normalizeDBShape(o){
  ...
  db.categorie=[...new Set((db.categorie||[]).map(x=>String(x||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'it',{sensitivity:'base'}));
  return db;
}
...
async function getArticlePics(a){
  ...
  if(Array.isArray(a.foto) && a.foto.length){
    for(const path of a.foto.slice(0,6)){
      ...
      if(url) cloudPics.push(url);
    }
  }
  if(cloudPics.length) return cloudPics;
...
async function downloadCurrentArticlePhotos(){
  ...
  const pics=await getArticlePics(art);
  ...
  for(let i=0;i<pics.length;i++){
    try{
      ...
      const ext=guessPhotoExtension(src, blob);
  ...
  if(!files.length){ toast('Download foto fallito'); return; }
  ...
  if(canNativeShare){
    try{
      ...
      toast(`Seleziona Galleria, Foto o File per salvare ${files.length} foto.`);
      return;
    }catch(err){
  ...
  toast(ok===files.length
    ...
    : (ok
      ? `Avviate ${ok} foto su ${files.length}. Se il browser ne blocca qualcuna, usa Condividi/Salva dal telefono.`
...
async function syncArticlePhotosToCloud(artId, codice, pics){
  ...
  for(let i=0;i<normalized.length;i++){
    ...
    // Se è già un path Supabase tipo CODICE/1.jpg, non ricaricare: usalo direttamente
    ...
    try{
      ...
      fileObj=new File([blob], `${cleanCode}-${i+1}.${ext}`, {type: blob.type || 'image/jpeg'});
    }catch(_e){
      continue;
    }
  ...
  if(savedPaths.length){
    const rows=savedPaths.map((path,idx)=>({prodotto_id: artId, path, ordine: idx}));
...
const BRAND_PRESETS=['Louis Vuitton','Gucci','Chanel','Dior','Alexander McQueen','Balenciaga','Versace'];
...
function syncBrandField(value=''){
  ...
  const known=BRAND_PRESETS.includes(clean);
  if(clean && !known){
    sel.value='__ALTRO__';
...
function renderCategorySettings(){
  ...
  box.innerHTML=cats.length?cats.map(c=>`<div class="row" style="grid-template-columns:1fr auto"><div class="main"><div class="t">${esc(c)}</div><div class="small">${savedSet.has(normalizeCategoryName(c))?'Categoria salvata nelle impostazioni':'Categoria rilevata da articoli già esistenti'}</div></div><div><button class="btn danger smallish" type="button" data-action="deleteCategory" data-name="${esc(c)}">Elimina</button></div></div>`).join(''):'<div class="small">Nessuna categoria salvata.</div>';
  if(hint) hint.textContent='Se elimini una categoria, viene tolta anche dagli articoli che la usavano. Gli articoli restano salvati e finiscono in Senza categoria.';
}
...
function renderArt(){
  ...
  const items=db.articoli.filter(a=>{
    ...
    const okCat=!catFilter || String(a.categoria||'').trim()===catFilter;
    return okText && okCat;
  ...
  if(q){
    const cards=items.map(a=>{
      ...
      return `<div class="artCard tight" data-open="art" data-id="${a.id}">
        ...
        <div class="artMeta"><div class="t">${esc(a.modello||a.codice||'-')}</div><div class="s">${esc(a.brand||'Brand non indicato')} • ${esc(a.categoria||'Senza categoria')}</div>${promo}</div>
        <div class="artPrice">${money(currentPrice(a))}</div>
        <div class="artActions"><button class="btn orderAdd small" type="button" data-action="addArtToOrder" data-id="${a.id}">Aggiungi a ordine</button></div>
      </div>`;
    }).join('');
  ...
  if(artBrowseState.level==='articoli' && artBrowseState.categoria && artBrowseState.brand){
    const scoped=items.filter(a=>String(a.categoria||'').trim()===artBrowseState.categoria && String(a.brand||'').trim()===artBrowseState.brand);
  ...
  if(artBrowseState.level==='brands' && artBrowseState.categoria){
    const scoped=items.filter(a=>String(a.categoria||'').trim()===artBrowseState.categoria);
    const groups=new Map();
    scoped.forEach(a=>{
      const brand=String(a.brand||'Senza brand').trim()||'Senza brand';
  ...
  const groups=new Map();
  items.forEach(a=>{
    const categoria=String(a.categoria||'Senza categoria').trim()||'Senza categoria';
    if(!groups.has(categoria)) groups.set(categoria,{categoria,count:0,latest:0,brands:new Set()});
    const g=groups.get(categoria); g.count+=1; g.latest=Math.max(g.latest,Number(a._ts||0)); if(a.brand) g.brands.add(String(a.brand).trim());
  });
...
function renderOrd(){
  ...
  el.innerHTML=items.map(o=>{
    ...
    return `<div class="row" data-open="ord" data-id="${o.id}" style="grid-template-columns:1fr auto">
      <div>
        <div class="t">${esc(c?c.nome:'-')} • <span class="${orderPaidStatus(o)?'pricePaid':'priceHot'}">${money(o.totale||0)}</span></div>
...
function renderAll(){ renderHome(); renderArt(); renderCli(); renderOrd(); renderFinanze(); renderCategorySettings(); }
...
async function duplicateCurrentArticle(){
  if(!currentArtId){ toast('Apri prima un articolo'); return; }
  const db=loadDB();
  const a=db.articoli.find(x=>x.id===currentArtId);
  if(!a){ toast('Articolo non trovato'); return; }
  const clone=JSON.parse(JSON.stringify(a));
  clone.id='';
  clone.codice='';
  hide('mArtView');
  await openArtEdit(null);
  currentArtDuplicateData={
    foto: Array.isArray(a?.foto) ? a.foto.filter(Boolean).slice(0,6) : [],
    photoIds: Array.isArray(a?.photoIds) ? a.photoIds.filter(Boolean).slice(0,6) : []
  };
...
async function saveArt(){
  try{
    ...
    const obj={
      ...
      foto: Array.isArray(old?.foto) ? old.foto.filter(x=>typeof x==='string' && !String(x).startsWith('data:')).slice() : [],
      photoIds: Array.isArray(old?.photoIds) ? old.photoIds.filter(Boolean).slice() : []
    };
...
function openOrdView(id){
  ...
  rows.innerHTML=(o.righe||[]).map(r=>`<div class="row" style="grid-template-columns:1fr auto"><div><div class="t">${esc(r.codice||'')} • ${esc(r.modello||'')}</div><div class="s">${money(r.prezzo||0)}</div></div><div class="pill">Riga</div></div>`).join('') || `<div class="small">Nessuna riga</div>`;
  const ordPics=mergeUniquePics(o.foto||[], o.fotoArticoli||[], o.fotoManuali||[]);
...
async function saveCli(){
  ...
  const nomeLower=safeLower(nome);
  const exists = db.clienti.find(c =>
    safeLower(c?.nome) === nomeLower
    && c.id !== currentCliId
  );
  if(exists){
...
async function openOrdEdit(id, prefillArticleId=null){
  ...
  currentOrdExistingManualPhotos=Array.isArray(o?.fotoManuali) ? o.fotoManuali.filter(Boolean).slice(0,12) : [];
...
async function saveOrd(){
  ...
  const selectedFiles=Array.from(photoInput?.files||[]).filter(f=>f && String(f.type||'').startsWith('image/'));
  const manualBase=Array.isArray(currentOrdExistingManualPhotos) ? currentOrdExistingManualPhotos.filter(Boolean) : [];
  const maxNew=Math.max(0, 12 - mergeUniquePics(articlePics, manualBase).length);
  const newManual=[];
  for(const file of selectedFiles.slice(0,maxNew)){
    try{ newManual.push(await compressImageFile(file, 1400, 0.76)); }catch(err){ console.warn('Compressione foto ordine fallita', err); }
  }
  const manualPics=mergeUniquePics(manualBase, newManual);
  const allOrderPics=mergeUniquePics(articlePics, manualPics);
  const obj={
    ...
    righe: ordRows.slice(),
    fotoArticoli: articlePics,
    fotoManuali: manualPics,
    foto: allOrderPics,
    totale: tot,
    _ts: Date.now()
  };
  ...
  if(!saveDB(db)) return;
  hide('mOrdEdit');
  toast(allOrderPics.length ? 'Ordine salvato con foto' : 'Ordine salvato');
  if(cloudEnabled()){
    try{
...
function importDB(file){
  ...
  r.onload=async()=>{
    try{
      ...
      updateShippingNotifications(o,'json-import');
      localStorage.setItem(KEY, JSON.stringify(o));
      renderAll();
      document.getElementById('importHint').textContent = 'Import OK. Database ricaricato.';
      toast('Import OK');
    }catch(e){
...
document.addEventListener('click',(ev)=>{
  ...
  if(a==='duplicateArt') return duplicateCurrentArticle();
...
async function upsertCloudArticle(art){
  ...
  return {...art, id:data.id, foto: cloudFoto.length?cloudFoto:(art.foto||[]), _cloud:true};
}
...
async function upsertCloudClient(cli){
  ...
  if(isUuid(cli.id)){
    ...
    if(error && cli.email){
      const existing=await sb.from('clienti').select('*').eq('email', cli.email).maybeSingle();
      if(existing.data && !existing.error){
        ({data,error}=await sb.from('clienti').upsert({...payload,id:existing.data.id}).select('*').single());
      }
    }
  }
  if(error) throw error;
...
async function pullCloudToLocal(opts={}){
  ...
  try{
    ...
    (fotos||[]).forEach(r=>{
      ...
      if(r.path) arr.push(r.path);
      fotoByProd.set(r.prodotto_id, arr);
    });
    for (const p of (prod||[])) {
      const hasRows = Array.isArray(fotoByProd.get(p.id)) && fotoByProd.get(p.id).length;
      if (!hasRows) {
        const fallbackPaths = await listCloudFotoPathsByCode(p.sku||'');
