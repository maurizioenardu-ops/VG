...
function normalizeDBShape(o){
  ...
  db.articoli=db.articoli.map(x=>({
    ...
    costoUsdPromo:firstFiniteNumber(x?.costoUsdPromo, x?.promoUsd, x?.usdPromo, x?.costoPromoUsd, 0),
...
function mergeArticleRecords(prev,item){
  ...
  merged.costoUsdPromo=firstFiniteNumber(merged?.costoUsdPromo, merged?.promoUsd, merged?.usdPromo, item?.costoUsdPromo, item?.promoUsd, item?.usdPromo, prev?.costoUsdPromo, prev?.promoUsd, prev?.usdPromo, 0);
...
function promoFinalPrice(a){
  const qual=qualityFromCode(a?.codice||'');
  const promoUsd=Number(a?.costoUsdPromo||a?.promoUsd||a?.usdPromo||0);
  if(promoUsd>0){
    const r=calcFromUsd(promoUsd, qual);
    return Number(r.sell||0);
  }
  return Number(a?.prezzoPromo||0);
}
function promoValid(a){
  if(!a.promoAttiva) return false;
  const p=promoFinalPrice(a);
...
function currentPrice(a){
  return promoValid(a) ? Number(promoFinalPrice(a)) : Number(a.prezzoVendita||0);
}
...
async function openArtEdit(id){
  ...
  if(!a){
    ['a_cod','a_mod','a_desc','a_forn','a_taglia','a_variante','a_colore','a_mis','a_usd','a_promo_usd','a_promo_price','a_promo_date','a_note','a_post','a_qual','a_eur','a_sell','a_final','a_margin','a_margin_pct']
  ...
  set('a_mis', a?.misura); set('a_usd', a?.costoUsd||''); set('a_promo_usd', a?.costoUsdPromo||'');
...
function renderArtAutoFields(){
  ...
  if(!hasUsd){
    ...
    const draft={
      ...
      costoUsdPromo:Number(document.getElementById('a_promo_usd').value||0),
  ...
  const promoUsd=Number(document.getElementById('a_promo_usd').value||0);
  const promoAuto=promoUsd>0 ? calcFromUsd(promoUsd, qual).sell : Number(document.getElementById('a_promo_price').value||0);
  document.getElementById('a_promo_price').value = promoAuto ? String(promoAuto) : '';
  const promoPrice=promoAuto;
  const promoDate=document.getElementById('a_promo_date').value.trim();
  const a={codice:code, prezzoVendita:r.sell, promoAttiva:promoOn, costoUsdPromo:promoUsd, prezzoPromo:promoPrice, scadenzaPromo:promoDate};
  ...
  const draft={
    ...
    modello:document.getElementById('a_mod').value.trim(),
    promoAttiva:promoOn, costoUsdPromo:promoUsd, prezzoPromo:promoPrice, scadenzaPromo:promoDate,
...
async function duplicateCurrentArticle(sourceId=null){
  ...
  document.getElementById('a_promo_usd').value=clone.costoUsdPromo||'';
...
async function saveArt(){
  try{
    ...
    const obj={
      ...
      misura: document.getElementById('a_mis').value.trim(),
      costoUsd: usd,
      costoUsdPromo: promoUsd,
      costoEur: r.costo,
      prezzoVendita: r.sell,
      promoAttiva: document.getElementById('a_promo_on').checked,
      prezzoPromo: promoAuto,
      scadenzaPromo: document.getElementById('a_promo_date').value.trim(),
...
['a_cod','a_brand','a_brand_custom','a_mod','a_mis','a_usd','a_promo_usd','a_promo_on','a_promo_price','a_promo_date','a_forn','a_desc','a_taglia','a_variante','a_colore'].forEach(id=>{
  const el=document.getElementById(id);
  ...
  try{
    ...
    const db={version:'v6',createdAt:new Date().toISOString(),
      articoli:(prod||[]).map(p=>{
        ...
        const pulledCostoUsdPromo=firstFiniteNumber(meta.costoUsdPromo, meta.promoUsd, meta.usdPromo, local?.costoUsdPromo, local?.promoUsd, local?.usdPromo, 0);
