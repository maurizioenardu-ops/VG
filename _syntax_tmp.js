...
function normalizeDBShape(o){
  ...
  db.articoli=db.articoli.map(x=>({
    ...
    costoUsdPromo:firstFiniteNumber(x?.costoUsdPromo, x?.promoUsd, x?.promoUSD, x?.prezzoUsdPromo, x?.prezzoPromoUsd, 0),
...
function mergeArticleRecords(prev,item){
  ...
  merged.costoUsdPromo=firstFiniteNumber(merged?.costoUsdPromo, merged?.promoUsd, merged?.promoUSD, item?.costoUsdPromo, item?.promoUsd, item?.promoUSD, prev?.costoUsdPromo, prev?.promoUsd, prev?.promoUSD, 0);
...
function promoSalePrice(a){
  const promoUsd=Number(a?.costoUsdPromo||a?.promoUsd||0);
  if(promoUsd>0){
    const qual=qualityFromCode(a?.codice||'');
    return Number(calcFromUsd(promoUsd, qual).sell||0);
  }
  return Number(a?.prezzoPromo||0);
}
function promoValid(a){
  if(!a.promoAttiva) return false;
  const p=promoSalePrice(a);
...
function currentPrice(a){
  return promoValid(a) ? Number(promoSalePrice(a)) : Number(a.prezzoVendita||0);
}
...
function buildPost(a){
  ...
  if(promoValid(a)){
    ...
    lines.push(`💶 ${money(currentPrice(a))}`);
...
function buildCloudArticleMeta(art){
  return {
    ...
    costoUsdPromo: Number(art?.costoUsdPromo||art?.promoUsd||0),
...
async function openArtEdit(id){
  ...
  set('a_promo_date', a?.scadenzaPromo); set('a_promo_price', a?.costoUsdPromo||a?.promoUsd||'');
...
function renderArtAutoFields(){
  ...
  if(!hasUsd){
    ...
    const draft={
      ...
      costoUsdPromo:Number(document.getElementById('a_promo_price').value||0),
      prezzoPromo:0,
  ...
  const promoUsd=Number(document.getElementById('a_promo_price').value||0);
  const promoDate=document.getElementById('a_promo_date').value.trim();
  const promoSell=promoUsd>0 ? calcFromUsd(promoUsd, qual).sell : 0;
  const a={codice:code, prezzoVendita:r.sell, promoAttiva:promoOn, costoUsdPromo:promoUsd, prezzoPromo:promoSell, scadenzaPromo:promoDate};
  ...
  const draft={
    ...
    modello:document.getElementById('a_mod').value.trim(),
    promoAttiva:promoOn, costoUsdPromo:promoUsd, prezzoPromo:promoSell, scadenzaPromo:promoDate,
...
async function duplicateCurrentArticle(sourceId=null){
  ...
  document.getElementById('a_promo_price').value=clone.costoUsdPromo||clone.promoUsd||'';
...
async function saveArt(){
  try{
    ...
    const promoDate=document.getElementById('a_promo_date').value.trim();
    if(promoOn){
      if(!promoDate){ toast('Scadenza promo obbligatoria'); return; }
      if(promoDate < todayStr()){ toast('Promo scaduta: disattivala o cambia data'); return; }
      if(!(promoUsd>0)){ toast('Inserisci il prezzo USD promo'); return; }
      if(promoSell >= r.sell){ toast('Il prezzo promo finale non può superare il prezzo normale'); return; }
    }
    ...
    const obj={
      ...
      prezzoVendita: r.sell,
      promoAttiva: promoOn,
      prezzoPromo: promoSell,
      scadenzaPromo: promoDate,
  ...
  try{
    ...
    const db={version:'v6',createdAt:new Date().toISOString(),
      articoli:(prod||[]).map(p=>{
        ...
        const pulledCostoUsdPromo=firstFiniteNumber(meta.costoUsdPromo, meta.promoUsd, local?.costoUsdPromo, local?.promoUsd, 0);
