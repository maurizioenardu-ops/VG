...
function openTrackParcel(){
  ...
  const schemeUrl=`parcel://track/${encodeURIComponent(code)}`;
  const intentUrl=`intent://track/${encodeURIComponent(code)}#Intent;scheme=parcel;package=bruno.parcel;end`;
  const start=Date.now();
  const onBlur=()=>{ window.__parcelOpened=true; };
  window.__parcelOpened=false;
  window.addEventListener('blur', onBlur, {once:true});
  try{ window.location.href=schemeUrl; }catch(_e){}
  setTimeout(()=>{
    if(window.__parcelOpened) return;
    try{ window.location.href=intentUrl; }catch(_e){}
  }, 280);
  setTimeout(()=>{
    if(window.__parcelOpened) return;
    if(Date.now()-start < 2600) window.open(fallback,'_blank','noopener');
  }, 1100);
}
